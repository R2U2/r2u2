...
{
	double diff_angle = abs_diff_angle(instr.sig_addr->d, 
...
{
	filter_movavg_update_data(instr.filt_data_struct.movavg, instr.sig_addr->i);
...
{
	double rate = filter_rate_update_data(instr.sig_addr->d,
...
{
	atomics_vector[instr.atom_addr] = instr.sig_addr->b;
}

void op_int(instruction_t instr)
{
	atomics_vector[instr.atom_addr] = 
		compare_int[instr.comp](instr.sig_addr->i, 
								instr.comp_const.i);
}
...
{
	atomics_vector[instr.atom_addr] = 
		compare_double[instr.comp](instr.sig_addr->d, 
								   instr.comp_const.d);
}
