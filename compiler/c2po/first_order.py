...
class FormulaLexer(sly.Lexer):
    ...
    tokens = { TL_GLOBAL, TL_FUTURE, TL_HIST, TL_ONCE, TL_UNTIL, TL_RELEASE, TL_SINCE, 
               ...
               NEWLINE, NUMERAL, COMMA, LBRACK, RBRACK, LPAREN, RPAREN, COLON }
...
class FormulaParser(sly.Parser):
    ...
    def expr(self, p):
        if p[7] not in self.bounds:
            log.error(
                MODULE_CODE, 
                f"Predicate '{p[7]}' not found in bounds file", 
...
def parse_fo(
...
) -> Optional[tuple[cpt.Expression, dict[str, int]]]:
    """Parse contents of input and returns corresponding first-order expression and guard bounds on success, else returns None."""
    ...
    expr: cpt.Expression = parser.parse(lexer.tokenize(contents))
